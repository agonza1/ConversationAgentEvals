...
Use this as the first and default local path. It starts the API, Pipecat service, and web app together, then you can run a benchmark from the focused runner at `/benchmarks`.
...
For this minimal path, leave `RTC_ASR_BASE_URL` empty unless you are intentionally testing live microphone ASR. The benchmark runner can use transcript, action trace, and final-state evidence without a live ASR service.
...
The command prints the local service URLs and the web URL to open:

```text
LiveSalesAIPresenter dev stack
   API      http://localhost:<api-port>
   Web      http://localhost:<web-port>
   Pipecat  http://localhost:<pipecat-port>

Open the operator UI at http://localhost:<web-port>
Use the printed URL above if default ports were busy.
```

If a default port is busy, the command prints the replacement port for that run. Open the printed web URL, go to `/benchmarks`, and run a scenario. Success means benchmark suites load, the scenario completes, and the report shows transcript, action trace, final state, scores, and evidence artifacts.

### ASR Contract For Conversation Demos

The minimal local demo does not require live microphone ASR. Live conversation demos use `rtc-asr` as the speech-to-text provider contract; see [issue #78](https://github.com/agonza1/ConversationAgentEvals/issues/78) for the tracked endpoint and configuration work. Any local mock or transcript fixture is demo support, not the production ASR contract.

Live Pipecat ASR expects an rtc-asr service reachable at `RTC_ASR_BASE_URL`, and streams audio to `RTC_ASR_STREAM_PATH` using the Local STT v1 WebSocket protocol. The default stream path is `/v1/stt/stream`; the default health path is `/health`.
...
Everything below is optional for the first local demo.

### Docker Startup

Use Docker when you want a production-style container startup:
...
The Docker path mirrors production service shape: Compose builds the API, Pipecat, and web images, starts Postgres, runs the one-shot seed service, and then starts the API, worker, Pipecat, and web services behind health checks. Local state remains mounted under `./storage` and the named `postgres_data` volume.
...
Most conversation eval tools grade tone or transcript quality. This project is aimed at a stricter question: did the AI agent actually complete the job?
...
- Provides queue/worker lifecycle primitives for long-running ASSERT jobs.
