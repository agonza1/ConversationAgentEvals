...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if call_id and not isinstance(raw_response.get('proof'), dict):
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Run the Phase 1 ConversationAgentEvals example against a running Agentic Contact Center target.'
    )
    parser.add_argument(
        '--scenario',
        type=Path,
    ...
    parser.add_argument(
        '--skip-submit',
        action='store_true',
        help='Collect and normalize ACC evidence without submitting it to ConversationAgentEvals.',
    )
