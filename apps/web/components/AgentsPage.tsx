...
> = {
  ...
  voice: [
    ...
    { value: 'builtin_sample_voice', label: 'Built-in generalist voice agent', group: 'Built-in samples' },
  ],
};
...
function targetLabel(target: FormTarget) {
  ...
  if (target === 'builtin_sample_voice') return 'Built-in generalist voice agent';
  return 'Legacy saved voice replay';
}

function isBuiltInTarget(target: FormTarget) {
    ...
    <div className="agents-modal-backdrop" role="presentation" onClick={onClose}>
      ...
      >
        ...
        <form className="agents-modal-form" onSubmit={(event) => void handleSubmit(event)}>
          ...
          {target === 'builtin_sample_voice' ? (
            <div className="agents-form-notice" role="note">
              <strong>Built-in generalist voice evaluation</strong>
              <span>
                Runs a Pipecat tester against a separate Pipecat generalist agent through rtc-asr, a configured OpenAI-compatible or Codex LLM, and Kokoro. Transcript, score, state, timing, media, and vCon come only from this run; unavailable evidence stays unproven. This is local synthetic media, not a browser, SIP, or phone call.
              </span>
            </div>
          ) : null}
          <label>
  ...
  async function onCreate(values: AgentFormState) {
    ...
    try {
      await createAgent({
        ...
        environment: isBuiltInTarget(values.target) || isSavedReplayTarget(values.target) ? 'local' : values.environment,
